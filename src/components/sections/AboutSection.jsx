...
import { Sparkles, Compass, Code2, Terminal, Cpu, Tv, Gamepad2, Quote } from 'lucide-react';
    ...
    <section id="about" className="container mx-auto py-20 px-6 text-center">
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 text-left">
        ...
        <div className="lg:col-span-7">
          <SpotlightCard className="p-8 sm:p-10 h-full flex flex-col justify-between border-teal-200/60 dark:border-teal-800/60 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-2xl shadow-xl">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-teal-600 dark:text-teal-400 font-semibold text-sm uppercase tracking-wider">
                <Sparkles className="w-4 h-4" /> Code & Curiosity
              </div>
            ...
            {/* Interest Badges */}
            <div className="mt-8 pt-6 border-t border-teal-100 dark:border-teal-800/40 flex flex-wrap gap-3">
              {interests.map((item) => {
                const IconComponent = item.icon;
                return (
                  <span
                    key={item.label}
                    className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold bg-teal-500/10 text-teal-800 dark:text-teal-300 border border-teal-500/20"
                  >
                    <IconComponent className="w-3.5 h-3.5 text-teal-500" />
                    {item.label}
                  </span>
                );
              })}
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <SpotlightCard className="p-8 sm:p-10 flex-1 flex flex-col justify-between border-teal-200/60 dark:border-teal-800/60 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-2xl shadow-xl">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-teal-600 dark:text-teal-400 font-semibold text-sm uppercase tracking-wider">
                <Compass className="w-4 h-4" /> Mindset & Goal
              </div>
          ...
          {/* Quote Card */}
          <div className="relative group p-6 rounded-2xl border border-teal-300/40 dark:border-teal-700/40 bg-linear-to-r from-teal-500/15 via-emerald-500/10 to-cyan-500/15 backdrop-blur-xl shadow-lg">
            <Quote className="w-6 h-6 text-teal-500/40 mb-2" />
            <p className="text-sm sm:text-base italic font-medium text-teal-950 dark:text-slate-100 leading-snug">
